...
Usage examples from workspace root:
    python Backend/scripts/train_classification.py --model custom --data_dir classification_dataset
...
def main(args):
    ...
    train_counts = {
        'bird': len(tf.io.gfile.glob(str(data_dir / 'train' / 'bird' / '*'))),
        'drone': len(tf.io.gfile.glob(str(data_dir / 'train' / 'drone' / '*'))),
    }
    ...
    if args.model == 'custom':
        ...
        out_name = out_dir / 'best_custom_cnn.h5'
        lr = 1e-3
    else:
        model = build_transfer_model(input_shape=img_size+(3,))
        out_name = out_dir / 'best_transfer_model.h5'
        lr = 1e-3
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint(str(out_name), save_best_only=True)
    ]
    ...
    print('Saved best model as', str(out_name))
